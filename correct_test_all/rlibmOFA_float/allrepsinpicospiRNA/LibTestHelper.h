...
double tensorfloat19exp(float x) {
  ...
  int exact = mpfr_round_nearest_away(__MPFR_ELEM__, mvalRes, mvalInput);
  
  ChangeMPFRParameter();
  
  exact = mpfr_round_nearest_away(mpfr_check_range, mvalRes, exact);
  exact = mpfr_round_nearest_away(mpfr_subnormalize, mvalRes, exact);
  
  RestoreMPFRParameter();
  
  double result = mpfr_get_d(mvalRes, MPFR_RNDZ);
...
double roundToTensorfloat19(double d) {
  ...
  int exact = mpfr_round_nearest_away(mpfr_set_d, mvalRes, d);
  exact = mpfr_round_nearest_away(mpfr_check_range, mvalRes, exact);
  exact = mpfr_round_nearest_away(mpfr_subnormalize, mvalRes, exact);
    ...
    for (unsigned long count = 0x0; count < upperlimit; count++) {
      ...
      if (oracleResult != roundedRes) {
        ...
        if (wrongResult < 10) {
          ...
          fprintf(lfd, "rounding mode index is RNA\n");
